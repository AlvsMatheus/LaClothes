...
if (!stripeSecretKey) {
  ...
  throw new Error(
    "ERRO FATAL: STRIPE_SECRET_KEY não está definida nas variáveis de ambiente."
  );
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const item of clientItems) {
        ...
        return NextResponse.json(
          {
            ...
            details:
              "O item no carrinho não existe (ou foi removido) da coleção 'products'.",
          },
          { status: 400 }
        );
      }
      ...
      if (!officialProduct || officialProduct.stock < item.quantity) {
        return NextResponse.json(
          {
            error: `Item "${
              officialProduct?.nome || item.id
            }" indisponível ou estoque insuficiente.`,
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // Lembre-se de remover o "T" que causou o erro anterior
      line_items,
      mode: "payment",
    ...
    return NextResponse.json(
      {
        error:
          "Não foi possível iniciar o pagamento. Verifique o console do servidor.",
        details: errorMessage, // Envia a mensagem de erro real
      },
      { status: 500 }
    );
  }
