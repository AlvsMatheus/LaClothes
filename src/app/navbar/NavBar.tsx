...
const NavBar = () => {
  ...
  useEffect(() => {
    ...
    setFiltered(produtos.filter((p) => p.nome?.toLowerCase().startsWith(q)));
  }, [query, produtos]);
    ...
    <>
      <article className="fixed top-0 left-0 w-full z-50 flex flex-col bg-[#e8e8e8]">
        ...
        <div className="md:hidden flex items-center justify-between h-20 px-4 relative">
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2">
            <Link href="/">
              <Image className="h-10 w-30" src={logoImg.imgPath} alt="Logo" />
            </Link>
          </div>
        </div>
      ...
      >
        ...
        <ul className="flex flex-col p-4 gap-4 text-white">
          <li>
            <Link href="/" onClick={() => setSideMenu(false)}>
              Início
            </Link>
          </li>
          <li>
            <Link href="/user" onClick={() => setSideMenu(false)}>
              Perfil
            </Link>
          </li>
          <li>
